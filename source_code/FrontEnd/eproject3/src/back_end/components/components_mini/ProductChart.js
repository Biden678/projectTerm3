...
const ProductChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:5044/api/Product');
        ...
        const filteredOrders = orderData.filter(order => new Date(order.dateFrom) >= lastMonth);

        // Count the occurrences of each product in the orders
        const productQuantities = {};
        filteredOrders.forEach(order => {
          const productName = order.type_Insurance; // Assuming product name is stored in type_Insurance
